...
STDAPI ExtractText(const wchar_t* fn, int maxLength, wchar_t* out)
{
	...
	try
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			if (spIFilter)
			{
					...
					while (moreChunks)
					{
							...
							while (moreText)
							{
								...
								else
								{
									...
									case FILTER_E_NO_TEXT:
										std::cout << "GetText: The current chunk does not contain text.";
										return hr;
									default:
										std::cout << "GetText: Unexpected error.";
										return hr;
									}
								}
							}
						...
						else
						{
							...
							case FILTER_E_PASSWORD:
								std::cout << "GetChunk: Password or other security-related access failure.";
								return hr;
							case FILTER_E_ACCESS:
								std::cout << "GetChunk: Access failure.";
								return hr;
							default:
								std::cout << "GetChunk: Unexpected error.";
								return hr;
							}
						}
					}
				...
				else
				{
					...
					{
					case E_FAIL:
						std::cout << "Init : File filter for this file type not found.";
						return hr;
					
					case E_INVALIDARG:
						std::cout << "Init : Count and contents of attributes do not agree.";
						return hr;

					case FILTER_E_PASSWORD:
						std::cout << "Init : Access has been denied because of password protection or similar security measures.";
						return hr;
					case FILTER_E_ACCESS:
						std::cout << "Init : Unable to access file.";
						return hr;
					default:
						std::cout << "Init : Unexpected error.";
						return hr;
					}
				}
			}
			...
			{
				std::cout << "Init : Unable to query for IFilter interface.";
				return E_FAIL;
			}
		}
		else
		{
			...
			{
			case E_ACCESSDENIED:
				std::cout << "LoadIFilter: Access denied to the filter file.";
				return hr;
			case E_HANDLE:
				std::cout << "LoadIFilter: Invalid handle, probably due to a low-memory situation.";
				return hr;
			case E_INVALIDARG:
				std::cout << "LoadIFilter: Invalid parameter.";
				return hr;
			case E_OUTOFMEMORY:
